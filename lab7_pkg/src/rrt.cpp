...
RRT::RRT(): rclcpp::Node("rrt_node"), gen((std::random_device())()) {
    ...
    use_rrt_pub = this->create_publisher<std_msgs::msg::Bool>("/use_rrt",1); //Boolean if we should activate RRT, returned from check_to_activate_rrt
    ...
    string dynamic_obs_topic = "/dynamic_obs_odom";
    ...
    dynamic_obs_sub = this->create_subscription<nav_msgs::msg::Odometry>(
    dynamic_obs_topic, 1, std::bind(&RRT::opponent_callback, this, std::placeholders::_1));

    previous_time = rclcpp::Clock().now();
    RCLCPP_INFO(rclcpp::get_logger("RRT"), "%s\n", "Created new RRT Object.");

    //Read in spline points
    std::string file_name = "src/Final_Race_F1_tenth/pure_pursuit/scripts/spline.csv";
    //std::string file_name = "src/pure_pursuit/scripts/spline.csv";
...
void RRT::scan_callback(const sensor_msgs::msg::LaserScan::ConstSharedPtr scan_msg) {
    ...
    if (current_time.seconds() - time_last_opponent_msg.seconds() > opponent_topic_timeout){
        seen_opponent = false;
    }

    if (seen_opponent == true){
        std::vector<float> obstacle_velocity = opponent_velocity;
        std::vector<float> obstacle_position= {opponent_position[0],opponent_position[1]};

        //Get range of scan at opponenet heading
        bool gaining = check_if_gaining_on_opponent(obstacle_position, obstacle_velocity);
        float opponent_relative_heading = atan2(obstacle_position[1], obstacle_position[0]);

        int idx = (opponent_relative_heading - scan_msg->angle_min) / scan_msg->angle_increment;
        obstacle_position[0] = scan_msg->ranges[idx] * cos(scan_msg->angle_increment * idx + scan_msg->angle_min);
        obstacle_position[1] = scan_msg->ranges[idx] * sin(scan_msg->angle_increment * idx + scan_msg->angle_min);

        // If we are getting further away from the opponent, remove them from the occupancy grid
        if(gaining == false){
            std::cout<<"Removing opponent"<<std::endl;
            listed_data = dynamic_obstacle(obstacle_position, obstacle_velocity, listed_data);
        }
    }
...
void RRT::opponent_callback(const nav_msgs::msg::Odometry::ConstSharedPtr odom){
    // Update the opponent position when we recieve a new message from IPM
    opponent_position[0] = odom->pose.pose.position.x;
    opponent_position[1] = odom->pose.pose.position.y;
    opponent_position[2] = odom->pose.pose.position.z;

    opponent_velocity[0] = odom->twist.twist.linear.x;
    opponent_velocity[1] = odom->twist.twist.linear.y;
    opponent_velocity[2] = odom->twist.twist.linear.z;

    seen_opponent = true; // Set the flag that we saw an opponent
    time_last_opponent_msg = rclcpp::Clock().now(); // And capture the time when it happens
}

/// FUNCTIONS FOR DETECTING RRT ON/OFF ///
...
void RRT::check_to_activate_rrt(std::vector<signed char> &obstacle_data){
    try{
        ...
        int goal_spline_idx = find_spline_index(global_goal.pose.pose.position.x, global_goal.pose.pose.position.y);
        ...
        for(int i=0;i<path_data.size();i++){
            if(path_data[i]==100 && i < obstacle_data.size() && obstacle_data[i]==100){
                //hit_count++;
                rrt_use_it = true;
                break;
            }
        ...
        auto path_grid = nav_msgs::msg::OccupancyGrid();
        path_grid.info.resolution=  resolution;
        path_grid.info.width=occu_grid_x_size;
        path_grid.info.height=occu_grid_y_size;
        std::string frame_id="map";
        path_grid.header.frame_id=frame_id;
        path_grid.header.stamp=rclcpp::Clock().now();
        path_grid.info.origin = current_car_pose.pose.pose;
        ...
        path_grid.info.origin.position.x-= shift_in_global_coords[0];
        path_grid.info.origin.position.y-= shift_in_global_coords[1];
        path_grid.data= path_data;
        grid_path_pub->publish(path_grid);
...
bool RRT::check_if_gaining_on_opponent(std::vector<float> obstacle_position, std::vector<float> obstacle_velocity){
    float velocity_threshold = -0.5;
    bool gaining = false;
    float distance = sqrt(pow(abs(obstacle_position[0]),2) + pow(abs(obstacle_position[1]),2));
    if(distance < 0.3){
        gaining = true;
    }
    if(obstacle_velocity[0] < velocity_threshold && distance < 4.0){
        gaining = true;
    }
    return gaining;
}

std::vector<signed char> RRT::dynamic_obstacle(std::vector<float> obstacle_position, std::vector<float> obstacle_velocity,std::vector<signed char> flat_grid){

    float grid_x_point= obstacle_position[0] /resolution + center_x;
    float grid_y_point= obstacle_position[1] /resolution + center_y;

    //Remove Dynamic Obstacle from the Occupancy Grid
    for(int i=0;i<flat_grid.size();i++){

        int ix = i % occu_grid_y_size;
        int iy = floor(i/occu_grid_x_size);

        //Remove points within 8 pixels of the dynamic obstacle
        if(sqrt(pow(abs(ix - grid_x_point),2) + pow(abs(iy - grid_y_point),2)) < 15){
            flat_grid[i] =0;
        }
    }
    
    /*//predict the future pose of the opponent car at multiple time steps
    std::vector<std::vector<float>> opponent_pose_list;
    float prev_x= obstacle_position[0];
    float prev_y= obstacle_position[1];
    float dt = 0.1;
    for(int i=0;i<10;i++){
        float x_step = prev_x + obstacle_velocity[0] * dt;
        float y_step = prev_y + obstacle_velocity[1] * dt;
        prev_x = x_step;
        prev_y = y_step;
        opponent_pose_list.push_back(x_step,y_step);
    }*/

    //Fill in grid based on the expected position of the dynamic obstacle
